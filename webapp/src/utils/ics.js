// this repo was clone from https://github.com/nwcell/ics.js/blob/master/ics.js
// Cameron Trando takes no credit for this, and he is thankful for the original creator, Travis Krause
// and Kyle Hornberg
            ...
            if (rrule) {
                if (!rrule.rrule) {
                    ...
                    if (typeof rrule.byday !== 'undefined') {
                        ...
                        for (let d of rrule.byday) {
                            if (IGNORE_DAYS.indexOf(d) !== -1) {
                                return false;
                            }
                            if (!(d in WEBREG_TO_BYDAY_VALUES)) {
                                throw new Error("Recurrence rrule 'byday' values must include only the following: 'Su', 'M', 'Tu', 'W', 'Th', 'F', 'Sa'");
                            }
                        }
                    }
                }
            ...
            if (rrule) {
                if (rrule.rrule) {
                    ...
                    if (rrule.byday && rrule.byday.length > 0) {
                        rruleString += ';BYDAY=' + rrule.byday.map(day => WEBREG_TO_BYDAY_VALUES[day]).join(',');
                    }
                }
            }
