...
class Class:
    def __init__(self, cursor, ID):
        ...
        # self.data contains all the possible subclass keys it could have
        # If it doesn't have one of a type it has None for the field
        # subclass_key is something with _KEY, subclass_index is a number to its table
    ...
    def make_subclasses(self, cursor, ID):
        """
        We must make a new Subclass for each time slot.
        :param cursor:
        :param ID:
        :return: a list of all the subclasses of this type
        """
        retList = []
        ...
        for entry in day_time_pairs:
            new_data = data.copy()
            new_data['DAYS'] = entry[0]
            new_data['TIME'] = entry[1]
            retList.append(Subclass(new_data))
        return retList
...
class Subclass:
    def __init__(self, data):
        ...
        self.interval = data['TIME'] if data['TIME'] else DefaultTimeIntervalCollection()
